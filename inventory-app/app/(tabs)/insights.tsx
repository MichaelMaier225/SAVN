...
export default function InsightsScreen() {
  ...
  const { buckets, summary, productRows } = useMemo(() => {
    const now = new Date()
    ...
    const productTotals = activeProducts.map(product => {
      const productTx = summaryTx.filter(
        tx => tx.productId === product.id
      )
      const productRevenue = productTx
        .filter(tx => tx.type === "sale")
        .reduce((sum, tx) => sum + tx.amount, 0)
      const productExpenses = productTx
    ...
    return {
      ...
      productRows: productTotals.sort(
        (a, b) => b.revenue - a.revenue
      ),
    }
  }, [products, range, t, transactions])
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.chartCard}>
          <View style={styles.productHeaderRow}>
            ...
            <Text style={styles.productHeaderValue}>
              {t("revenue")}
            </Text>
            <Text style={styles.productHeaderValue}>
              {t("expenses")}
